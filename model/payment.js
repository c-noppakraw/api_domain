...
const search_payment = async (keyword) => {
    const sql = `SELECT * FROM price WHERE domain_name LIKE '%${keyword}%'`;
    const data_list = await db.query(sql)
    return data_list;
}

const get_payment = async (id_payment) => {
    const sql = `SELECT * FROM price WHERE id = '${id_payment}'`;
    const data_list = await db.query(sql)
    return data_list;
}

const post_payment = async (id_payment, payload) => {
...
const delete_payment = async (id_payment, payload) => {
